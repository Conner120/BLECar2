...
void emer() {
  if (emergancy) {
    if ((millis() % 375) == 0) {
      if (red) {
        for (int i = 0; i < 5; i++) {
          digitalWrite(seirnRed[i], LOW);
        }
        for (int i = 0; i < 5; i++) {
          digitalWrite(seirnBlue[i], HIGH);
        }
        red = false;
      } else {
        red = true;
        for (int i = 0; i < 5; i++) {
          digitalWrite(seirnRed[i], HIGH);
        }
        for (int i = 0; i < 5; i++) {
          digitalWrite(seirnBlue[i], LOW);
        }
      }
    }
  } else {
    ...
    while ( Serial.available() )
    {
      ...
      if (c == '/') {
        ...
        if (str[0] == 'r') {
          ...
          float motorValue = str.substring(1, str.length() - 1).toFloat() * 8.0;
          sendDebugMesg(str.substring(1, str.length() - 1));
          sendDebugMesg("left Motor run at: " + String(motorValue) + "%");
          if ((motorValue / 100 * 1023) < 0.000) {
          ...
          int motorLeft = 0;
          int motorRight = 0;
          sendDebugMesg(String(motorLeft));
        } else {
    ...
    while ( ble.available() )
    {
      ...
      if (c == '/') {
        ...
        if (str[0] == 'r') {
          ...
          Serial.println(str.substring(2, 6));
          int motorLeft = str.substring(2, 6).toInt();
          int motorRight =  str.substring(7, 12).toInt();
          sendDebugMesg(String(motorLeft));
          sendDebugMesg(String(motorRight));
          if (motorLeft < 0.000) {
            motorLeft = motorLeft * -1;
            inMotion = true;
            motorgo(1, 1, motorLeft);
            sendDebugMesg(String(motorLeft));
          } else if (motorLeft > 0.000) {
            inMotion = true;
            motorgo(1, 2, motorLeft);
          } else {
            motorOff(1);
            inMotion = false;
          }
          if (motorRight < 0.000) {
            ...
            motorgo(0, 1, motorRight);
            sendDebugMesg(String(motorRight));
          } else if (motorRight > 0.000) {
            inMotion = true;
            motorgo(0, 2, motorRight);
          } else {
            motorOff(0);
            inMotion = false;
          }
...
void checkCommand(String str) {
  ...
  if (str.indexOf(F("emergancyMode/")) > -1) {
    ...
    headLightOff();
  } else if (str.indexOf(F("areadCur/")) > -1) {
    Serial.println("Curent 0 : " + String(analogRead(2)));
    Serial.println("Curent 1 : " + String(analogRead(3)));

  } else if (str.charAt(2) == 'B') {
    Serial.println("button");
    if (str.charAt(3) == '1' && str.charAt(4) == '0') {
      headLightOn();
      Serial.println("lights on");
    } else if (str.charAt(3) == '2' && str.charAt(4) == '1') {
      headLightOff();
      Serial.println("lights off");
    } else if (str.charAt(3) == '5' && str.charAt(4) == '1') {
      tone(A8, toneSet);
      Serial.println("Horn High");
    } else if (str.charAt(3) == '5' && str.charAt(4) == '0') {
      noTone(A8);
      Serial.println("Horn Off");
    } else if (str.charAt(3) == '3' && str.charAt(4) == '1') {
      emergancy = true;
      Serial.println("Emergancy Mode Active");
    } else if (str.charAt(3) == '4' && str.charAt(4) == '0') {
      emergancy = false;
      Serial.println("Emergancy Mode Not Active");
    }
  }
